...
- **Thread principal**: espera mensagem do sensor de velocidade via mensage queue e aciona a thread da câmera via zbus como trigger para executar a leitura de uma placa no formato Mercosul, recebendo a placa de volta pelo zbus, valida a placa verificando se o formato da placa condiz com algum dos formatos de placas do mercosul e envia para a thread do display os valores velocidade, eixos e placa para a thread display, caso placa valida, envia a placa, caso contrario envia "INVALIDO":
